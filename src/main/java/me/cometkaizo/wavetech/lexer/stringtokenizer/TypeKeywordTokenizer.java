...
public class TypeKeywordTokenizer extends StringTokenizer {

    @Override
    public boolean accepts(LineReader reader) {
        return Keywords.isPrimitiveTypeKeyword(reader.currentWord());
    }

    @Override
    public Token tokenize(LineReader reader) {
        if (!accepts(reader)) throw new IllegalArgumentException();
        return new Token(Keywords.parsePrimitiveTypeKeyword(reader.currentWord()));
    }
}
