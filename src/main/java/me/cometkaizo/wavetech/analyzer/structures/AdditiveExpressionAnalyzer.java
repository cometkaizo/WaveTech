...
public class AdditiveExpressionAnalyzer implements ProgramContextAnalyzer {
    private final AdditiveExpression expression;
    boolean analyzed = false;

    public AdditiveExpressionAnalyzer(AdditiveExpression expression) {
    ...
    private static void analyze(List<Diagnostic> problems, ResourcePool resources, MultiplicativeExpression expr) {
        expr.getAnalyzer().analyze(problems, resources);
    }

    private MultiplicativeExpression getFirstExpr() {
    ...
    private static ClassStructure getResultType(ClassStructure leftType, ClassStructure rightType) {
        return leftType == INT && rightType == INT ? INT : null;
    }
