...
import static me.cometkaizo.wavetech.parser.BuiltInClasses.DOUBLE;
import static me.cometkaizo.wavetech.parser.BuiltInClasses.INT;

public class MultiplicativeExpressionAnalyzer implements ProgramContextAnalyzer {
    private final MultiplicativeExpression expression;
    boolean analyzed = false;

    public MultiplicativeExpressionAnalyzer(MultiplicativeExpression expression) {
    ...
    private static void analyze(List<Diagnostic> problems, ResourcePool resources, PrefixExpression expr) {
        expr.getAnalyzer().analyze(problems, resources);
    }

    private PrefixExpression getFirstExpr() {
    ...
    private static ClassStructure getResultType(ClassStructure leftType, ClassStructure rightType) {
        return leftType == INT && rightType == INT ? DOUBLE : null;
    }
