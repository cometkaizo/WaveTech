...
public class RelationalExpressionAnalyzer implements ProgramContextAnalyzer {
    private final RelationalExpression expression;
    boolean analyzed = false;

    public RelationalExpressionAnalyzer(RelationalExpression expression) {
    ...
    private static void analyze(List<Diagnostic> problems, ResourcePool resources, ShiftExpression expr) {
        expr.getAnalyzer().analyze(problems, resources);
    }

    private ShiftExpression getFirstExpr() {
    ...
    private static ClassStructure getResultType(ClassStructure leftType, ClassStructure rightType) {
        return leftType == INT && rightType == INT ? BOOLEAN : null;
    }
